...
// Статический метод для запроса на любом из соединений
    static QVector<QPair<QString, int>> loadWordsWithConnection(QSqlDatabase &db);
...
private:
    ...
    QSqlQuery executeWordsQuery();
    QVector<QPair<QString, int>> readWordsFromQuery(QSqlQuery &query);

    QSqlQuery executeSearchQuery(const QStringList &words, int limit);
    QVector<SearchResult> readSearchResults(QSqlQuery &query);
};
