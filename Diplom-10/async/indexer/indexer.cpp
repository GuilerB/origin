...
// Минимальный размер для проверки кодировки
...
// Максимальный размер файла для индексации (100 МБ)
constexpr qint64 MAX_FILE_SIZE_BYTES = 100LL * 1024 * 1024;
...
// Ограничение размера файла — не загружать файлы > 100 МБ
    if (fileInfo.size() > MAX_FILE_SIZE_BYTES) {
        qWarning() << "File too large, skipping:" << filePath
                    << "(" << fileInfo.size() << "bytes)";
        return;
    }
...
// Логирование
    if (content.isEmpty()) {
        qWarning() << "Empty content, skipping:" << filePath;
        return;
    }

    QStringList tokens = Tokenizer::tokenize(content);
    if (tokens.isEmpty()) {
        qWarning() << "No tokens extracted, skipping:" << filePath;
        return;
    }
    ...
    if (docId < 0) {
        qWarning() << "Failed to get document ID for:" << filePath;
        return;
    }
