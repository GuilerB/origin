...
// WordListLoader — в отдельном потоке

WordListLoader::WordListLoader(QObject *parent) : QObject(parent)
{
// Соединение открывается в loadWordList() в рабочем потоке
}
...
WordListLoader::~WordListLoader()
{
    ...
    QSqlDatabase::removeDatabase("thread_connection");
}

QVector<QPair<QString, int>> WordListLoader::loadWordList()
{
// Соединение в рабочем потоке
    m_threadDb = QSqlDatabase::addDatabase("QPSQL", "thread_connection");
    ...
    if (!m_threadDb.open()) {
        emit error("Не удалось подключиться к БД из рабочего потока: " + m_threadDb.lastError().text());
        return {};
    }
...
void SearcherWindow::loadWordList()
{
    ...
    QMetaObject::invokeMethod(loader, [loader]() {
